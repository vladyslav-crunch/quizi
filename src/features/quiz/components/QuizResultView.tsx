...
export function QuizResultView({
  ...
  const scorePercent = totalQuestions > 0 ? Math.round((firstAttemptCorrectCount / totalQuestions) * 100) : 0
    ...
    <div className="flex min-h-svh items-center justify-center bg-background p-3 text-foreground sm:p-4">
      <Card className="w-full max-w-md shadow-lg">
        ...
        <CardContent className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground sm:text-base">{scorePercent}% correct on the first attempt</p>
          <p className="text-sm text-muted-foreground sm:text-base">Wrong on first try: {firstAttemptWrongCount}</p>
        </CardContent>
